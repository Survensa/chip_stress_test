...
_DEFAULT_CONFIG_FILE = "../../Configs/configFile.yaml"
...
def default_config_reader():
    try:
        if not os.path.exists(_DEFAULT_CONFIG_FILE):
            logging.error("The config file does not exist! exiting now! ")
            sys.exit(0)
        with io.open(_DEFAULT_CONFIG_FILE, 'r') as f:
            test_config_dict = yaml.safe_load(f)
            test_config = TestConfig(test_config_dict)
        return test_config
    except Exception as e:
...
def summary_log(test_result: dict, test_config: object, completed: bool, analytics_json: dict):
    if completed:
