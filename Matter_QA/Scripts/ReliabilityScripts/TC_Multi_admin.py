...
from Matter_QA.Library.HelperLibs.utils import convert_args_dict
...
class TC_Multi_admin(MatterQABaseTestCaseClass):
    def build_fabrics(self, i: int):
        ...
        th2_fabric_admin = fabric_certificate_authority.NewFabricAdmin(vendorId=0xFFF1, fabricId=self.th1.fabricId + i)
        fabric_dut_node = self.dut_node_id + i
        fabric_node_id = self.th1.nodeId + i
        return {"fabric_dut_node_id": fabric_dut_node, "fabric_object": th2_fabric_admin.NewController(fabric_node_id),
    ...
    async def cluster_on_off_state(self):
    ...
    async def on_dev(self, controller, fabric):
        await controller.SendCommand(nodeid=fabric["fabric_dut_node_id"], endpoint=1,
                                     payload=Clusters.OnOff.Commands.On())

    async def off_dev(self, controller, fabric):
        await controller.SendCommand(nodeid=fabric["fabric_dut_node_id"], endpoint=1,
    ...
    async def test_stress_test_multi_fabric(self):
        ...
        for i in range(1, int(dict_args["--fabrics"])):
            ...
            await self.pair_the_nodes(fabric_details)
            await asyncio.sleep(2)
        ...
        for i in range(self.test_config_dict["general_configs"]["iteration_number"]):
            ...
            logging.info(
                f"Random fabric chosen is {fabric_random_1['name']},random controller used for ON operation is {controller_random_1}")
            await self.cluster_on_off_state()
            await asyncio.sleep(3)
            await self.on_dev(controller_random_1, fabric_random_1)
            await self.cluster_on_off_state()
            await asyncio.sleep(3)
            fabric_random_2 = random.choice(fabrics)
            controller_random_2 = random.choice(fabric_random_2["controllers"])
            logging.info(
                f"Random fabric chosen is {fabric_random_2['name']}, random controller used for OFF operation is {controller_random_2}")
            await self.off_dev(controller_random_2, fabric_random_2)
            await self.cluster_on_off_state()
            await asyncio.sleep(3)
