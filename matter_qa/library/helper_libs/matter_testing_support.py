...
def read_config_from_file(filename: str) -> dict[str, bool]:
    try:
...
def convert_args_to_matter_config(args: argparse.Namespace) -> MatterTestConfig:
    ...
    config.reliability_tests_config = {} if args.reliability_tests_arg is None else read_config_from_file(args.reliability_tests_arg)
...
def parse_matter_test_args(argv: List[str]) -> MatterTestConfig:
    ...
    reliability_tests_group = parser.add_argument_group(title="reliability test arugment", description="reliability test case global arguments set")
    reliability_tests_group.add_argument('--reliability-tests-arg', type=pathlib.Path,
                            help="JSON string or file path representing extra parameters")
...
def default_matter_test_main(argv=None, **kwargs):
    ...
    with TracingContext() as tracing_ctx:
        ...
        with runner.mobly_logger():
            if (not kwargs.get("do_not_commision_first")):
